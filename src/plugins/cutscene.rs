...
pub enum CutsceneAction {
    ...
    FadeAndZoom {amount: f32, duration: f32, start: Option<Vec3>, reversed: bool},
    MovePlayer(Vec3),
    ...
    loop {
        ...
        match action {
            ...
            CutsceneAction::ZoomCamera(amount, duration, start) => {
                ...
                let target = Vec3::new(player.0.translation.x, 10. - *amount, player.0.translation.z + 10. - *amount);
                camera_transform.translation = start.lerp(target, smooth_t);
             }
            CutsceneAction::FadeAndZoom {amount, duration, start, reversed} => {
                let start = start.get_or_insert(camera_transform.translation);
                let timer = cutscene_sequence.timer.get_or_insert(Timer::from_seconds(*duration, TimerMode::Once));
                timer.tick(time.delta());
                let t = (timer.elapsed_secs() / *duration).clamp(0.0, 1.0);
        ...
        if cutscene_sequence.timer.as_ref().is_some_and(|timer| !timer.is_finished()) {
            return;
        } else {
